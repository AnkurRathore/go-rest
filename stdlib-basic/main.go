...
func (ts *taskServer) taskHandler(w http.ResponseWriter, req *http.Request) {
	if req.URL.Path == "/task/" {
		...
		if req.Method == http.MethodPost {
			...
			http.Error(w, fmt.Sprintf("expected method GET, DELETE or POST at /task/,got %v", req.Method), http.StatusMethodNotAllowed)
		}
	} else {
...
func (ts *taskServer) createTaskHandler(w http.ResponseWriter, req *http.Request) {
	...
	// Types used internally to de-serialize the request
	// and response from/to json
...
func (ts *taskServer) dueHandler(w http.ResponseWriter, req *http.Request) {
	...
	if req.Method != http.MethodGet {
		http.Error(w, fmt.Sprintf("expect /due/<date>, got %v", req.URL.Path), http.StatusMethodNotAllowed)
		return
	}
	path := strings.Trim(req.URL.Path, "/")
	pathParts := strings.Split(path, "/")
